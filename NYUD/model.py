...
from utils import lrelu, conv_concat, scaled_tanh
...
class DSN(object):
    ...
    def build_model(self):
	    ...
	    self.fzy = self.sampler_generator(self.src_noise,self.src_labels)
            ...
            self.src_images = tf.placeholder(tf.float32, [None, 224, 224, 3], 'source_images')
            self.trg_images = tf.placeholder(tf.float32, [None, 224, 224, 3], 'target_images')
	    self.labels_gen = tf.placeholder(tf.float32, [None, self.no_classes], 'labels_gen')
	    ...
	    self.gen_trg_images_show = self.G(self.sampler_generator(self.src_noise[:16,:],self.labels_gen[:16], reuse=True), self.labels_gen[:16], reuse=True, do_reshape=True)
	    ...
	    # Trg const loss
	    self.const_loss = tf.reduce_mean(tf.square(self.GE_trg - tf.scalar_mul(1./255.,self.trg_images))) * 15.0 
	    ...
	    self.DG_optimizer = tf.train.AdamOptimizer(self.learning_rate / 100.)
            ...
            rec_trg_images_summary = tf.summary.image('rec_trg_images', self.GE_trg, max_outputs=19)
            trg_images_summary = tf.summary.image('trg_images', self.trg_images, max_outputs=19)
            self.summary_op = tf.summary.merge([E_loss_summary, DE_loss_summary, 
                                                    G_loss_summary, DG_loss_summary,const_loss_summary,
